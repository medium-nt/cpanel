            ...
            <div class="card-body">
                ...
                <div class="row">
                    @if(auth()->user()->isAdmin())
                        ...
                        <a href="{{ route('marketplace_api.newOrder') }}"
                           class="btn btn-success mr-3 mb-3">Загрузить заказы с
                            API</a>

                        <a href="{{ route('marketplace_api.check_cancelled') }}"
                           class="btn btn-warning mr-3 mb-3">Проверить
                            отмененные заказы</a>
                    @endif

                    @if(auth()->user()->isAdmin() || auth()->user()->isManager())
                        <a href="{{ route('marketplace_orders.import_excel') }}"
                           class="btn btn-info mr-3 mb-3">
                            <i class="fas fa-file-excel mr-1"></i> Добавить
                            заказ через Excel
                        </a>
                    @endif
                </div>
                    ...
                    <div class="card">
                        <div class="position-relative">
                            ...
                            <div class="card-body">
                                ...
                                @if($orderStatus && $order->status != 3 && !auth()->user()->isManager())
