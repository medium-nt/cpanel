...
**Вешалки (Hanger) — настройка закройщика:**

- Колонка `users.hanger_id` (FK→hangers.id, nullable) — вешалка, на которую
  закройщик вешает раскроенные товары
- `User::hanger()` — связь belongsTo с моделью `Hanger`
- Закройщик может сменить вешалку через модалку на странице «Товары для
  пошива» (`MarketplaceOrderItemController::setHanger`, PUT set-hanger)
- При сдаче раскроя (`MarketplaceOrderItemController::completeCutting`,
  ...
  закройщика
- Только закройщики (`isCutter()`) могут менять вешалку — иначе 403 через
  `HangerPolicy`
- Справочник вешалок управляется администраторами в Настройках (аналог Полок)
