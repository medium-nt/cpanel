...
- **Проверка материалов в цехе (`hasMaterialsInWorkshop`):**
  - Проверяется доступность тканей (`Material::TYPE_FABRIC`) для швей с кроем
    и
    закройщиков
  - **Упаковочные материалы (`Material::TYPE_PACKAGING`) исключены из проверки
    ** —
    они списываются отдельным потоком упаковщика через `KioskService` и
    `StickerPrintingController`
  - Швеи без кроя (`seamstressNotCut`) пропускают проверку тканей — крой
    делает
    закройщик
