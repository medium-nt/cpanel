...
- **Лимит 100 заказов на FBS-поставку WB:** Wildberries API ограничивает
  ...
  один PATCH-запрос. Для Ozon лимита нет. UI-компонент `SupplyOrderSearch` общий
  для WB и Ozon, но проверка `marketplace_id === WbApiService::MARKETPLACE_ID` (
  =2)
  применяется только к WB. При попытке добавить 101-й заказ UI блокирует
  действие с
