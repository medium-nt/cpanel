...
- `InventoryService::canArchive(Material)` — проверяет, можно ли перевести
  материал в архив
- Условие: у материала НЕТ активных рулонов в таблице `rolls` (статусы
  `in_storage`, `shipped_to_workshop`, `in_workshop`)
- Материал можно перевести в «Архив» ТОЛЬКО из статуса «Нельзя заказать»
  (`is_active=false`) и при `canArchive() == true`
- Рулоны в статусе `completed` (даже с остатком `used < initial_quantity`) НЕ
  блокируют архивацию — completed = работа завершена, остаток уже учтён как
  брак/списание
- Материалы без рулонов вообще (старые, до эры рулонов) архивации НЕ
  препятствуют
- `MaterialController::update` — защита через redirect back при нарушении
  условий
  с ошибкой «Нельзя архивировать: у материала есть незакрытые рулоны (на складе,
  в пути или в цехе).»
...
- Перевод в архив возможен только когда НЕТ активных рулонов (на складе, в
  пути в цех, в цехе). Рулоны в статусе `completed` и материалы без рулонов
  archival не препятствуют
...
- `app/Services/InventoryService.php` — canArchive(Material) — проверка
  возможности перевода в архив (нет активных рулонов в статусах
  in_storage/shipped_to_workshop/in_workshop)
