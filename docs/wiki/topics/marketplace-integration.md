...
- `app/Services/MarketplaceOrderService.php` — сервис удаления заказов в
  FBO-поставках (методы `delete()` и `deleteNewOrdersBySupply()`)
- `app/Http\Controllers\MarketplaceOrderController.php` — контроллер с методом
  `destroyNewBySupply()` для массового удаления заказов
...
- `app/Policies/MarketplaceSupplyPolicy.php` — политика `deleteOrders()` для
  контроля доступа к удалению заказов
