...
**Отвязка не готовых заказов в FBO-поставках:** администраторы могут массово
отвязывать заказы от поставки, когда она в статусе 13 ("На сборке")

- **Критерий:** заказы без короба (`box_id IS NULL`) и в работе (`status != 0`)
- Отвязка НЕ удаляет заказы — только устанавливает `supply_id = null`
- Заказы в коробах (`box_id != null`) всегда остаются в поставке
- "Новые" заказы (`status = 0`) не затрагиваются
