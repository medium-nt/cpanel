...
- **Admin-only операции с рулонами:** ручное списание метража рулона
  (`RollController@writeOff`) — доступно только администраторам через страницу
  рулона (`/megatulle/rolls/show/{id}`), создаёт Order с `type_movement=10`,
  влияет на `Roll::current_quantity`, логируется в канал `materials`
