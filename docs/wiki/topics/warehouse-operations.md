...
- Wildberries API ограничивает эндпоинт
  ...
  PATCH-запрос. Для Ozon лимита нет.
- UI-блокировка в `SupplyOrderSearch::attachOrder()`: компонент общий для WB и
  Ozon, но проверка `marketplace_id === WbApiService::MARKETPLACE_ID` (=2)
  применяется
  только к WB. При привязке заказа к WB-поставке проверяется
