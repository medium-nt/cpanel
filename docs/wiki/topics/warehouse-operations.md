...
**canArchive — защита перевода в архив (17.07.2026):**

- `InventoryService::canArchive(Material)` — проверяет, можно ли перевести
  материал в архив
- Условие: у материала НЕТ активных рулонов (статусы `in_storage`,
  `shipped_to_workshop`, `in_workshop`)
...
- Материал можно перевести в «Архив» только из статуса «Нельзя заказать»
  (`is_active=false`)
- Рулоны в статусе `completed` (даже с остатком) НЕ блокируют архивацию
- Материалы без рулонов (старые) archival не препятствуют
...
**Mobile UX improvements (marketplace_order_items.index):**

- **Dropdown «Действия» для мобильных:** кнопки действий закройщика (получить
  заказ, взять весь стек, печать, сменить вешалку) собраны в один dropdown на
  мобильных устройствах
- Швея видит 1 пункт dropdown, закройщик — 4 пункта
- Dropdown размещён внутри блока фильтров (`col-12 only-on-smartphone`),
  на десктопе используется отдельный блок кнопок
- **Минимизация блока «Итого»:** на мобильных убрана обёртка
  `.card > .card-body`,
  теперь компактный inline `col-12 mb-2 small`
...
- `app/Http/Controllers/MarketplaceOrderItemController.php` — метод
  `setHanger` (PUT set-hanger), `completeCutting` (проставляет hanger_id), reset
  date filter для швей/закройщиков
- `resources/views/hangers/*` — views справочника
- `resources/views/marketplace_order_items/index.blade.php` — dropdown действий
  на
  мобильных, фильтр по датам (скрыт для швей/закройщиков кроме done), мобильная
  минимизация блока «Итого»
...
### Утилизация брака (WarehouseOfItemController)

**Бизнес-смысл:** Утилизация бракованных товаров через страницу «Забрать брак из
цеха»
(`/megatulle/warehouse_of_item/status_change_scan?from=16&to=19`).

**Статусы утилизации:**

- 16 — «Помечено на утилизацию» (начальный статус)
- 19 — «Утилизировать» (промежуточный статус для сканера)
- 17 — «Утилизировано» (финальный статус)

**Три потока утилизации:**

1. **Стандартный flow через сканер (16 → 19 → 17):**
    - Кладовщик сканирует товары со статусом 16
    - Livewire-компонент `complete()` переводит в статус 19
    - Товар попадает в список «Утилизировать»
    - Финальное сканирование переводит в статус 17

2. **Массовая утилизация админом из цеха (16 → 17 напрямую):**
    - Кнопка «Утилизировать все» на странице сканера (
      `status_change_scan?from=16&to=19`)
    - Доступна только админу (`auth()->user()->isAdmin()`)
    - POST-роут `warehouse_of_item.status_change_scan.utilize_defects`
    - Метод `WarehouseOfItemController::utilizeDefects()` — массовое
      `update(['status' => 17])` всех товаров со статусом 16
    - Логирование в канал `items`
    - Обходит промежуточный статус 19

3. **Массовая утилизация «переданных на осмотр в цех» (10 → 17 напрямую):**
    - Кнопка «Утилизировать все» на странице
      `/megatulle/warehouse_of_item/new_refunds`
    - Переводит ВСЕ товары со статусом 10 («На разборе» / «Переданные на осмотр
      в цех») в финальный статус 17
    - Доступна только админу (`auth()->user()->isAdmin()`)
    - POST-роут `warehouse_of_item.new_refunds.utilize_all`
    - Метод `WarehouseOfItemController::utilizeRefunds()` — массовое
      `update(['status' => 17])` всех товаров со статусом 10
    - Логирование в канал `items` (уникальный текст лога)
    - Статус 10 показывается в виджете «Переданные на осмотр в цех» на странице
      `/inspection`

**Файлы:**

- `app/Http/Controllers/WarehouseOfItemController.php` — методы
  `utilizeDefects()` (status 16) и `utilizeRefunds()` (status 10)
- `routes/warehouse_of_item.php` — POST-роуты `utilize_defects` (
  status_change_scan) и `utilize_all` (new_refunds)
- `resources/views/warehouse_of_item/status_change_scan.blade.php` — кнопка
  «Утилизировать все» (status 16)
- `resources/views/warehouse_of_item/new_refunds.blade.php` — кнопка
  «Утилизировать все» (status 10)
...
- **Admin-only операции с рулонами:**
    - Ручное списание метража рулона (`RollController@writeOff`) — доступно
      только
      администраторам через страницу рулона (`/megatulle/rolls/show/{id}`),
      создаёт
      Order с `type_movement=10`, влияет на `Roll::current_quantity`, логируется
      в
      канал `materials`
    - Закрытие рулона (`RollController@complete`) — доступно только
      администраторам
      через страницу рулона (`/megatulle/rolls/show/{id}`), переводит рулон в
      статус
      `COMPLETED`, фиксирует `completed_at`, `completed_by`, `shortage_quantity`
      (недостача/излишек)
...
**Лимит 100 заказов на FBS-поставку WB:** ✨ **НОВОЕ**

- Wildberries API ограничивает эндпоинт
  `/api/marketplace/v3/supplies/{supplyId}/orders` до 100 order_id за один
  PATCH-запрос
- UI-блокировка в `SupplyOrderSearch::attachOrder()`: перед привязкой заказа
  проверяется `$this->supply->marketplace_orders()->count() >= 100` → блокирует
  добавление 101-го заказа с сообщением «Достигнут лимит 100 заказов в поставке.
  Создайте новую поставку.»
- Safety-net в `WbApiService::addOrdersToSupply()`: при `count(order_id) > 100`
  запрос к WB API не делается, возвращаются все order_id как «не добавленные»
- Механизм отправки изменён: все order_id отправляются одним PATCH-запросом
  `{"orders": [...]}` вместо N последовательных (раньше пробивало rate-limit WB)
- **Только для FBS:** FBO-поставки WB через этот метод не проходят
...
- `app/Http/Controllers/WarehouseOfItemController.php` — метод `pickupScan()` —
  ...
  storage_barcode, метод `utilizeDefects()` — массовая утилизация брака админом
  (16→17)
- `routes/warehouse_of_item.php` — роут
  `warehouse_of_item.status_change_scan.utilize_defects`
- `resources/views/warehouse_of_item/status_change_scan.blade.php` — страница
  сканера утилизации с кнопкой «Утилизировать все» (admin only)
...
- **Утилизация брака:** три flow — стандартный через сканер (16→19→17),
  массовая утилизация админом из цеха (16→17 напрямую на `/status_change_scan`)
  и массовая утилизация «переданных на осмотр в цех» (10→17 на `/new_refunds`).
  Кнопки «Утилизировать все» доступны только админу на обеих страницах.
