...
## Route Groups
- `/megatulle/` + `auth` — базовые авторизованные роуты (users, shifts, transactions, workshops)
- `/megatulle/` + `auth` + `require_open_shift` — операционные роуты (materials, orders, marketplace, inventory)
- `routes/api.php` — webhooks (Telegram)
- `routes/kiosk.php` — интерфейс киоска
- `routes/console.php` — cron-задачи (подробнее в [maps/schedule.md](maps/schedule.md))
...
- [Warehouse Operations](topics/warehouse-operations.md) — склад, стеллажи, стикеры
