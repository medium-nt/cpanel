...
## [2026-06-16] update | shift-system-calendar-fix

- Исправлена проблема с миграцией на проде (MySQL 5.7): добавлена обработка
  `dropForeign` перед `dropUnique` для разрешения backing-индекса FK
- **Дополнение к бизнес-правилам:**
    - `ShiftSchedule::isDayOff()` → `shift_id === null` (метод проверки выхода)
    - Каскадное удаление смены не удаляет записи календаря — расписание цеха
      сохраняется, но сменность отменяется
    - Уточнена логика `ShiftService::canWorkToday()` — проверка выходного дня
      по `(workshop_id, shift_id null)` и принадлежности цеху сотрудника
    - Метод `fillCalendar()` теперь использует
      `updateOrCreate(['workshop_id', 'date'])`
