...
- **Methods:**
    ...
    - `static materialsQuantityBy(string $type)`
    - `static materialsQuantityByWorkshopPerShift(App\Models\User $user)`
...
- **Methods:**
  ...
  `static getRatingByDate(mixed $seamstress, mixed $startDate, mixed $endDate)`
    - `static getRatingOLD()`
    - `static getRating()`
    - `static getItemsForLabeling(Illuminate\Http\Request $request)`
...
- **Methods:**
  ...
  `static save_write_off(App\Http\Requests\SaveWriteOffMovementMaterialToWorkshopRequest $request)`
    - `static getCountNotShippedMovements(App\Models\User $user)`
    - `static getCountNotReceivedMovements(App\Models\User $user)`
    - `static getStickeredMarketplaceOrderItem()`
...
- **Methods:**
  ...
  `static transferEmployee(App\Models\User $user, App\Models\Shift $newShift, string $effectiveFrom)`
    - `static getMissingScheduleDates(int $days)`
...
- **Methods:**
    ...
    - `static getListSeamstressesWorkingToday()`
