...
- **Traits:** `HasFactory`
- **Fillable:** `marketplace_order_id`, `marketplace_item_id`,
...
- **Table:** `orders`
- **Traits:** `HasFactory`
- **Fillable:** `shift_id`, `type_movement`, `status`, `supplier_id`,
  `storekeeper_id`, `seamstress_id`, `cutter_id`, `otk_id`, `comment`,
  `marketplace_order_id`, `is_approved`, `completed_at`
- **Casts:** `id` → int
- **Relationships:**
    - `BelongsTo` user → User
